...
public:
    ...
    static WinComPtr createInst(REFCLSID clsid, DWORD dwClsContext, REFIID iid)
    {
        ...
        if(FAILED(createErr))
        {
            qWarning() << "Unable to create COM object -" << createErr;
        }
        return pResult;
    }
...
class COMMON_EXPORT WinComVariant
{
public:
...
class COMMON_EXPORT WinComInit : public QObject
{
    Q_OBJECT
public:
    WinComInit(QObject *pParent = nullptr);
    ~WinComInit();
